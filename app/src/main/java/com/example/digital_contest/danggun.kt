...
fun danggun() {
    ...
    val companyDetailData by companyDetailDataStore.companyDetailData.collectAsState(initial = CompanyDetail(emptyList()))
    ...
    val entries = remember(companyDetailData) {
        val danggunData = companyDetailData.data.find { it.target == "당근" }?.data ?: emptyList()
